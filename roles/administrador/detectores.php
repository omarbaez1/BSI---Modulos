...
<head>
    ...
    <script>
        ...
        function editDetector(detectorJson) {
            ...
            form.querySelector('[name="diasVenCalib"]').value = detector['DIAS_VEN_CALIB'];
            form.querySelector('[name="estadoCalibracion"]').value = detector['ESTADO_CALIBRACION'];
...
<div id="addForm" class="slide-container">
    ...
    <form method="POST" action="">
        ...
        <label for="diasVenCalib">Dias para vencer calibración:</label>
        <input type="text" name="diasVenCalib" id="diasVenCalib"><br><br>

        <label for="estadoCalibracion">Estado de Calibración:</label>
        <input type="text" name="estadoCalibracion" id="estadoCalibracion"><br><br>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($_POST['action'] == 'add') {
        $sql = "INSERT INTO detectores (SERIAL, CEDULA_TEC, FECHA_LLEGADA, PROCEDENCIA, VALOR, MARCA, MODELO, ESTUCHE, FECHA_CALIBRACION, FECHA_PROX_CALIBRACION,  DIAS_VEN_CALIB, ESTADO_CALIBRACION, QUIEN_LO_TENIA, UBI_ACT_EQUI, TEC_ASIG, ESTADO_EQUIPO, FECHA_ESTADO, OBSERVACION, OBSERVACION_PROSOIND, OBSERVACION_PERDIDOS, FECHA_ULT_VALID) 
        ...
        $sql = "UPDATE detectores SET CEDULA_TEC = :cedulaTec, FECHA_LLEGADA = :fechaLlegada, PROCEDENCIA = :procedencia, VALOR = :valor, MARCA = :marca, MODELO = :modelo, ESTUCHE = :estuche, FECHA_CALIBRACION = :fechaCalibracion, FECHA_PROX_CALIBRACION = :fechaProxCalibracion,  DIAS_VEN_CALIB = :diasVenCalib, ESTADO_CALIBRACION = :estadoCalibracion, QUIEN_LO_TENIA = :quienLoTenia, UBI_ACT_EQUI = :ubiActEqui, TEC_ASIG = :tecAsig, ESTADO_EQUIPO = :estadoEquipo, FECHA_ESTADO = :fechaEstado, OBSERVACION = :observacion, OBSERVACION_PROSOIND = :observacionProsoind, OBSERVACION_PERDIDOS = :observacionPerdidos, FECHA_ULT_VALID = :fechaUltValid 
                WHERE SERIAL = :serial";
    }
    ...
    $stmt->execute([
        ...
        'diasVenCalib' => $_POST['DIAS_VEN_CALIB'],
        'estadoCalibracion' => $_POST['ESTADO_CALIBRACION'],
    ...
    if ($resultados) {
        echo "<table border='1'>
                <tr>
                    ...
                    <th>Dias Vencidos Calibracion</th>
        ...
        foreach ($resultados as $fila) {
            ...
            echo "<tr>
                    ...
                    <td>{$fila['DIAS_VEN_CALIB']}</td>
                    <td>{$fila['ESTADO_CALIBRACION']}</td>
                    ...
                    <td>
                        <a href='#' onclick='editDetector(`$detectorJson`)'>Editar</a> |
                        <a href='#' onclick='confirmarEliminacion(\"{$fila['SERIAL']}\")'>Eliminar</a>
                    </td>
                  </tr>";
        }
        echo "</table>";
